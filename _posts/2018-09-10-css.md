...
| (llamado algunas veces   | del tipo especificado.   |                          |
| selector de etiqueta o   |                          | Selecciona \<p\>         |
| tipo)                    |                          |                          |
+--------------------------+--------------------------+--------------------------+
| Selector de              | El elemento en la página | \#my-id                  |
| identificación (ID)      | con el ID especificado   |                          |
|                          |  (en una página HTML     | Selecciona \<p           |
|                          | dada, solo se permite un | id="my-id"\> y \<a       |
|                          | unico elemento por ID).  | id="my-id"\>             |
+--------------------------+--------------------------+--------------------------+
| Selector de Clase        | Los elementos en la      | .my-class                |
|                          | página con la clase      |                          |
|                          | especificada (una clase  | Selecciona \<p           |
|                          | puede aparecer varias    | class="my-class"\> y \<a |
|                          | veces en una página).    | class="my-class"\>       |
+--------------------------+--------------------------+--------------------------+
| Selector de atributo     | Los elementos en una     | img[src]                 |
|                          | página con el atributo   |                          |
|                          | especificado.            | Selecciona \<img         |
|                          |                          | src="myimage.png"\> pero |
|                          |                          | no \<img\>               |
+--------------------------+--------------------------+--------------------------+
| Selector de Pseudo-clase | Los elementos            | a:hover                  |
|                          | especificados, pero solo |                          |
|                          | cuando esté en el estado | Selecciona \<a\>, pero   |
|                          | especificado, por        | solo cuando el puntero   |
|                          | ejemplo cuando el        | esté sobre el enlace.    |
...
1.  html {\
      font-size: 10px; /\* px quiere decir 'pixels': la base del tamaño
    de fuente es ahora de 1o pixeles\*/\
